...
echo "[ntp-server] using deterministic local clock (stratum 2 orphan)"
...
echo "[ntp-server] waiting for chronyd tracking..."
for i in $(seq 1 5); do
    chronyc tracking 2>/dev/null | grep -qE "Stratum[[:space:]]*: 2" && break
    sleep 1
done
