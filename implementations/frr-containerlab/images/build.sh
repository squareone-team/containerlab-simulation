...
docker build -t esi/fabric-browser:3.23 "$ROOT_DIR/fabric-browser"
docker build -t esi/auth-server:3.20 "$ROOT_DIR/auth-server"
docker build -t esi/vpn-node:3.20 "$ROOT_DIR/vpn-node"
